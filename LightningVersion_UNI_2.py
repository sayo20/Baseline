...
import IPython
from DataLoaderTemp import SoundDS
...
class ImagePredictionLogger(pl.Callback):
    ...
    def on_validation_epoch_end(self,trainer,pl_module):
        ...
        df.to_pickle("PlotSanityCheck_lpsMask-uni.pkl")



if __name__ == '__main__':
    wandb_logger = WandbLogger(project="Exploring-Baseline-Experiment2", config='config_UNI.yaml',name="uni-GU-LPS-V2-Ex2")#withouth window length
    ...
    trainer = Trainer(max_epochs=epoch,fast_dev_run=False,gpus=[3],callbacks=[early_stop_callback,checkpoint_callback,ImagePredictionLogger(val_samples)],logger=wandb_logger,auto_lr_find=True, profiler="simple") # max_epochs=epoch ,profiler="advanced", runs one batch to test that the model works
    ...
    trainer.save_checkpoint("Uni_experiments/uni-GU-LPS-V2-Ex2.ckpt")
    # #save for use in non-python environment
    script = model.to_torchscript()
    torch.jit.save(script, "Uni_experiments/uni-GU-LPS-V2-Ex2.pt")
