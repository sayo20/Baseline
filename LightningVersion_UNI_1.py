...
if __name__ == '__main__':
    wandb_logger = WandbLogger(project="Exploring-Baseline-Experiment1", config='config_UNI_1.yaml',name="uni-GU-LPS-V2-Ex1")#withouth window length
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename="uni-GU-LPS-V2-Ex1-{epoch:02d}-{val_loss:.2f}",
        save_top_k =-1,#
        mode="min",)
    ...
    trainer = Trainer(max_epochs=epoch,fast_dev_run=False,gpus=[1],callbacks=[early_stop_callback,checkpoint_callback],logger=wandb_logger,auto_lr_find=True, profiler="simple") # max_epochs=epoch ,profiler="advanced", runs one batch to test that the model works
    ...
    trainer.save_checkpoint("Uni_experiments/uni-GU-LPS-V2-Ex1.ckpt")
    # #save for use in non-python environment
    script = model.to_torchscript()
    torch.jit.save(script, "Uni_experiments/uni-GU-LPS-V2-Ex1.pt")
