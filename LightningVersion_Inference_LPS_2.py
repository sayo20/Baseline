...
from torchmetrics.functional.audio import scale_invariant_signal_distortion_ratio,signal_distortion_ratio,scale_invariant_signal_noise_ratio
...
#         print(output[:,-1,:].shape)
    ...
    def test_step(self,batch,batch_idx):
        ...
        df_test = pd.DataFrame(columns=["Mixed audio","Target audio", "Estimated audio","True mask","Estimated mask","Mixed spectra","Target spectra","Mixed phase","Az t","Az n", "snr","Mix type","reconstruction loss","si-sdr","si-snr","Test si-snr","stoi","sdr","pesq"])
        ...
        for i in range(len(data)):#cause is batch size len(data)
            ...
            test_sisnr = scale_invariant_signal_noise_ratio(true_audio[0,:],true_audio[0,:])
            ...
            print("sisnr: ",sisnr,test_sisnr,stoi,sdr,pesq)
            ...
            df_test.loc[i, "Test si-snr"] = test_sisnr
        ...
        torch.save(df_test_dict,"Spectral-experiments_LPS-3-results/spectra-LPS-V2-Ex3-TEST-"+str(batch_idx)+".pt")

    def test_dataloader(self):
        df_test = pd.read_csv("../Data/Libri-speech/Test-1k.csv")
        # df_val = df_val[:32]
        test_ds = SoundDS_LPS(df_test,config["Experiment"])
...
if __name__ == '__main__':
    wandb_logger = WandbLogger(project="Exploring-Baseline-Experiment3", config='config_LPS-2.yaml',name="spectra-LPS-V2-Ex3-TEST")#withouth window length
    ...
    input_size = config["Input_lps"] #train_features.shape[1]#1799 1542

    no_lstm_node = config["Number_of_nodes"]#512
    no_output_node = config["Label_lps"] #train_labels.shape[1]#257 not cuz we have 6 channels
    ...
    trainer = Trainer(max_epochs=epoch,fast_dev_run=False,gpus=[6],logger=wandb_logger,auto_lr_find=True, profiler="simple") # max_epochs=epoch ,profiler="advanced", runs one batch to test that the model works
