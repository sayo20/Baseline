...
class ImagePredictionLogger(pl.Callback):
    ...
    def on_validation_epoch_end(self,trainer,pl_module):
        ...
        df.to_pickle("PlotSanityCheck_lpsMask-bi-2.pkl")


if __name__ == '__main__':
    ####INFO: RUN 1: Normalized mixed spectra nd target spectra and  unbounded max
    wandb_logger = WandbLogger(project="Exploring-Baseline-Experiment2", config='config_BI.yaml',name="bi-GU-LPS-V2-Ex2")#Bilateral_gu_LpsMasks
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        save_top_k =-1,
        filename="bi-GU-LPS-V2-Ex2--{epoch:02d}-{val_loss:.2f}",#-{epoch:02d}-{val_loss:.2f}
        mode="min",)
    ...
    trainer = Trainer(max_epochs=epoch,fast_dev_run=False,gpus=[6],callbacks=[early_stop_callback,checkpoint_callback,ImagePredictionLogger(val_samples)],logger=wandb_logger,auto_lr_find=True, profiler="simple") # max_epochs=epoch ,profiler="advanced", runs one batch to test that the model works
    ...
    trainer.save_checkpoint("Bi_experiments/bi-GU-LPS-V2-Ex2.ckpt")
    script = model.to_torchscript()
    torch.jit.save(script, "Bi_experiments/bi-GU-LPS-V2-Ex2.pt")
