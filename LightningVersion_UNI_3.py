...
class ImagePredictionLogger(pl.Callback):
    ...
    def on_validation_epoch_end(self,trainer,pl_module):
        ...
        df.to_pickle("PlotSanityCheck_lpsMask-uni3.pkl")



if __name__ == '__main__':
    wandb_logger = WandbLogger(project="Exploring-Baseline-Experiment3", config='config_UNI_3.yaml',name="uni-GU-LPS-V2-Ex3")#withouth window length
    ...
    val_dl = torch.utils.data.DataLoader(val_ds, batch_size=config["batch_size"],shuffle=False)#64

    # start_time = timer()
    # train_features, train_labels,mixed_spectra_mag,mixed_spectra_phase,t_audio = next(iter(train_dl))
    # print("--- to do next iter: %s seconds ---" % (timer() - start_time))
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        save_top_k =-1,
        filename="uni-GU-LPS-V2-Ex3-{epoch:02d}-{val_loss:.2f}",#
        mode="min",)
    ...
    trainer = Trainer(max_epochs=epoch,fast_dev_run=False,gpus=[0],callbacks=[early_stop_callback,checkpoint_callback,ImagePredictionLogger(val_samples)],logger=wandb_logger,auto_lr_find=True, profiler="simple") # max_epochs=epoch ,profiler="advanced", runs one batch to test that the model works
    ...
    trainer.save_checkpoint("Uni_experiments/uni-GU-LPS-V2-Ex3.ckpt")
    # #save for use in non-python environment
    script = model.to_torchscript()
    torch.jit.save(script, "Uni_experiments/uni-GU-LPS-V2-Ex3.pt")

    ######################################
    # stats = pstats.Stats(profiler).sort_stats('cumtime')
    # stats.strip_dirs()
    # stats.print_stats()
    #do early stopping and profiling
