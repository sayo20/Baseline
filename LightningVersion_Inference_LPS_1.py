...
#         print(output[:,-1,:].shape)
    ...
    def test_step(self,batch,batch_idx):
        ...
        df_test = pd.DataFrame(columns=["Mixed audio","Target audio", "Estimated audio","True mask","Estimated mask","Mixed spectra","Target spectra","Mixed phase","Az t","Az n", "snr","Mix type","reconstruction loss","si-sdr","si-snr","stoi","sdr","pesq","Test-si-snr"])
        ...
        for i in range(len(data)):#cause is batch size len(data)
            ...
            test_si_snr = scale_invariant_signal_noise_ratio(true_audio[0,:],true_audio[0,:])
            ...
            df_test.loc[i, "Test-si-snr"] = test_si_snr
        ...
        torch.save(df_test_dict,"Spectral-experiments_LPS-1-results/spectra-LPS-V2-Ex1-TEST-"+str(batch_idx)+".pt")
...
if __name__ == '__main__':
    wandb_logger = WandbLogger(project="Exploring-Baseline-Experiment1", config='config_LPS-1.yaml',name="spectra-LPS-V2-Ex1-TEST")#withouth window length
    ...
    trainer = Trainer(max_epochs=epoch,fast_dev_run=False,gpus=[5],logger=wandb_logger,auto_lr_find=True, profiler="simple") # max_epochs=epoch ,profiler="advanced", runs one batch to test that the model works
