...
#         print(output[:,-1,:].shape)
    ...
    def train_dataloader(self):
        ...
        train_ds = SoundDS_LPS(df_train,config["Experiment"])
    ...
    def val_dataloader(self):
        ...
        # df_val = df_val[:32]
        val_ds = SoundDS_LPS(df_val,config["Experiment"])
...
class ImagePredictionLogger(pl.Callback):
    ...
    def on_validation_epoch_end(self,trainer,pl_module):
        ...
        df.to_pickle("PlotSanityCheck_lpsMask-lps-1.pkl")


if __name__ == '__main__':
    ####INFO: RUN 1: Normalized mixed spectra nd target spectra and  unbounded max
    wandb_logger = WandbLogger(project="Exploring-Baseline-Experiment1", config='config_LPS-1.yaml',name="spectra-LPS-V2-Ex1")#Bilateral_gu_LpsMasks
    ...
    # train_ds = SoundDS_LPS(df_train,config["Experiment"],"cuda") #0 for unilateral, 1 for bilateral
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor="val_loss",
        dirpath="Spectral_experiments/",
        save_top_k =-1,
        filename="spectra-LPS-V2-Ex1-{epoch:02d}-{val_loss:.2f}",#-{epoch:02d}-{val_loss:.2f}
        mode="min",)
    ...
    trainer = Trainer(max_epochs=epoch,fast_dev_run=False,gpus=[2],callbacks=[early_stop_callback,checkpoint_callback,ImagePredictionLogger(val_samples)],logger=wandb_logger,auto_lr_find=True, profiler="simple") # max_epochs=epoch ,profiler="advanced", runs one batch to test that the model works
    ...
    trainer.save_checkpoint("Spectral_experiments/spectra-Ex1.ckpt")
    script = model.to_torchscript()
    torch.jit.save(script, "Spectral_experiments/spectra-Ex1.pt")
